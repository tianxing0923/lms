...
router.get('/self', async (ctx, next) => {
  var data = await comment.listBySelf(ctx, ctx.query);
  ctx.status = 200;
  ctx.body = data;
});
...
router.post('/', async (ctx, next) => {
  var data = await comment.create(ctx, ctx.request.body);
  ctx.status = 201;
});

// 新增评论回复
router.post('/reply', async (ctx, next) => {
  var data = await comment.createReply(ctx, ctx.request.body);
  ctx.status = 201;
});
